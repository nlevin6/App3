Shader "NatureManufacture Shaders/Road/Standard_Metallic_Road_Transparent"
{
	...
	SubShader
	{
		Tags{ "RenderType" = "Custom" "Queue" = "Transparent+0" "Offset"="-2, -2" "ForceNoShadowCasting"="True" }
		Cull Back
		Offset  -2 ,-2
		CGPROGRAM
		...
		#pragma surface surf Standard keepalpha decal:blend finalcolor:ApplyOverCloud
		struct Input
		{
			float2 uv_texcoord;
			float3 viewDir;
		...
		void ApplyOverCloud(Input IN, SurfaceOutputStandard o, inout fixed4 color)
		{
			// Get ScreenPosition
			...
			// Sample the atmosphere and scattering mask (god rays)
			...
			// Premultiply some stuff if premultiply is on
    #if _ALPHAPREMULTIPLY_ON
			atm.scattering *= o.Alpha;
			atm.fog.rgb *= o.Alpha;
     #endif
    ...
    #else
			// In the case of an additive pass, just adding attenuating by the fog is enough
			color.rgb = lerp(color.rgb, 0, atm.fog.a);
    #endif
